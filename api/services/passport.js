...
passport.connect = function (req, query, profile, next) {
  ...
  db.User.findOne({'passports.provider': provider, 'passports.identifier': query.identifier.toString()}, function (err, u) {
    if (err) {
      return next(err);
    }
    if (u) {
    ...
    var passport = u && u.findPassport({provider: provider});
    ...
    if (!req.user) {
      ...
      // Action:   Get the user associated with the passport.
        ...
        user.save(function (err, passport) {
          if (err) {
            return next(err);
          }
      ...
      if (!passport) {        
        req.user.addPassport(query, function(err) {
          if (err) {
            return next(err);
          }
...
passport.disconnect = function (req, res, next) {
  ...
  user.removePassport({provider: provider}, function (error) {
    if (err) {
      return next(err);
    }
